...
<div class="container">
    <h1>Kalkulator PIT – Demo</h1>

    <p class="text-muted">To jest wersja demonstracyjna kalkulatora. Dane nie są zapisywane i będą widoczne tylko podczas tej sesji.</p>
    ...
    <form method="POST" action="{{ route('pit-calculator.demo.calculate') }}">
        @csrf

        <label for="income">Dochód</label>
        <input type="number" class="form-control" id="income" name="income" value="{{ old('income', $input['income'] ?? '') }}" required>

        <label for="expenses">Koszty uzyskania przychodu</label>
        <input type="number" class="form-control" id="expenses" name="expenses" value="{{ old('expenses', $input['expenses'] ?? '') }}" required>

        <label for="deductions">Ulgi i odliczenia</label>
        <input type="number" class="form-control" id="deductions" name="deductions" value="{{ old('deductions', $input['deductions'] ?? '') }}" required>

        <label for="tax_type">Typ opodatkowania</label>
        <select class="form-control" id="tax_type" name="tax_type" required>
            <option value="scale" {{ (old('tax_type', $input['tax_type'] ?? '') == 'scale') ? 'selected' : '' }}>Skala podatkowa (17%/32%)</option>
            <option value="flat" {{ (old('tax_type', $input['tax_type'] ?? '') == 'flat') ? 'selected' : '' }}>Podatek liniowy (19%)</option>
            <option value="ryczałt" {{ (old('tax_type', $input['tax_type'] ?? '') == 'ryczałt') ? 'selected' : '' }}>Ryczałt (8.5%)</option>
        </select>

        <label for="children">Liczba dzieci</label>
        <input type="number" class="form-control" id="children" name="children" value="{{ old('children', $input['children'] ?? '') }}">

        <label for="social_insurance">Składki społeczne</label>
        <input type="number" class="form-control" id="social_insurance" name="social_insurance" value="{{ old('social_insurance', $input['social_insurance'] ?? '') }}">

        <label for="health_insurance">Składki zdrowotne</label>
        <input type="number" class="form-control" id="health_insurance" name="health_insurance" value="{{ old('health_insurance', $input['health_insurance'] ?? '') }}">

        <div class="form-check mt-3">
            <input type="checkbox" class="form-check-input" id="is_married" name="is_married"
                {{ old('is_married', $input['is_married'] ?? false) ? 'checked' : '' }}>
            <label class="form-check-label" for="is_married">Wspólne rozliczenie małżeńskie</label>
        </div>

        <button type="submit" class="btn btn-primary">Oblicz PIT (demo)</button>
    </form>

    @isset($taxableIncome, $taxAmount)
        <div class="mt-4 alert alert-info">
            <h3>Wyniki kalkulacji (demo)</h3>
            <p><strong>Dochód do opodatkowania:</strong> {{ number_format($taxableIncome, 2) }} PLN</p>
            <p><strong>Podatek do zapłaty:</strong> {{ number_format($taxAmount, 2) }} PLN</p>
        </div>
    @endisset
</div>
