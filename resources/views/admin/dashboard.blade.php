    ...
    <div class="container py-4">
        ...
        <div class="mb-3">
            <form action="{{ route('dashboard.clear-history') }}" method="POST"
                onsubmit="return confirm('Czy na pewno chcesz usunąć wszystkie kalkulacje i historię?')">
                @csrf
                @method('DELETE')
                <button type="submit" class="btn btn-outline-danger">
                    <i class="bi bi-trash3"></i> Wyczyść wszystkie kalkulacje i historię
                </button>
            </form>
                ...
                <table class="table table-bordered table-striped align-middle text-center table-custom-bg">
                    ...
                    <tbody>
                        ...
                        @foreach ($histories as $index => $history)
                            @php
                                $details = is_array($history->previous_values)
                                    ? $history->previous_values
                                    : json_decode($history->previous_values, true) ?? [];
                                $newDetails = is_array($history->new_values)
                                    ? $history->new_values
                            ...
                            <tr>
                                ...
                                <td>{{ number_format($taxAmountToShow, 2, ',', ' ') }} zl</td>
                                <td class="text-start">
                                    ...
                                    <div class="collapse mt-2" id="detailsCollapse{{ $index }}">
                                        @if ($history->action === 'created')
                                            @php $dataToShow = $newDetails; @endphp
                                            @if (!empty($dataToShow))
                                                            ...
                                                            <li class="d-flex justify-content-between border-bottom py-1">
                                                                ...
                                                                <span>
                                                                    @if ($old !== null && $new !== null && $old != $new)
                                                                        <span
                                                                            class="text-danger">{{ displayValue($key, $old) }}</span>
                                ...
                                <td>
                                    <div class="d-flex justify-content-center gap-2">
                                        @if ($history->user)
                                            <a href="{{ route('tax-history.edit', $history->id) }}"
                    ...
                    neighbors.forEach(nbr => {
                        ...
                        if (dist < 180) {
                            ...
                            if (mouse.x !== null) {
                                ...
                                const mouseDist = Math.sqrt((mouse.x - midX) ** 2 + (mouse.y - midY) **
                                    2);
                                if (mouseDist < 130) {
